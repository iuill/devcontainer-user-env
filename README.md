...
テキストを貼り付けます。画像、動画、3Dモデルなど任意形式のファイル選択と
ドロップにも対応しています。
...
プレーンテキストとして表示します。サイズ上限を超えるテキストと、それ以外の
ファイルはダウンロード扱いになります。
...
- すべてのファイル形式を保存可能（PNG、JPEG、GIFとUTF-8プレーンテキストはプレビュー対応）
- 汎用ファイルの標準上限は500 MiB（`-max-file-bytes`で変更可能）
- 画像・テキストの標準上限は20 MiB（`-max-bytes`で変更可能）
- 画像は一辺20,000ピクセル、総ピクセル数1億まで
- アプリはユーザーが指定したファイル名を保存先に使用せず、拡張子だけを安全に引き継ぐ
- 削除できるのは保存ディレクトリ直下の通常ファイルだけ
